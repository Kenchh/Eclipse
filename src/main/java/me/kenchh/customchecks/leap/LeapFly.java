...
public class LeapFly extends Fly {
    ...
    public void move(PlayerMoveEvent e, DataProfile dp, Player p, double deltaY, double deltaH, double deltadeltaY, double deltadeltaH) {
        ...
        if(dp.onGround() == false) {
            ...
            /** : Non complex code, just flags if movement is very rapid and unusual. */
            if (deltaY >= 0.9 + jumpboost/10 && dp.airticks >= 3) {
            ...
            if (deltaY >= 0.30 && dp.airticks >= D_maxairticks && (deltadeltaY >= -0.15 && deltadeltaY <= 0.15)) {
            ...
            int AA_maxairticks = 40;
            if (ConnectionUtils.getPing(p) >= 80) {
                if(ConnectionUtils.getPing(p) < 250) {
                    AA_maxairticks = 40 * ConnectionUtils.getPing(p) / 80;
                } else {
                    AA_maxairticks = 40 * 250 / 80;
                }
            }
